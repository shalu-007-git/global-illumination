...
struct Block {
    Block(size_t id, float convergence = 0)
        : id(id), conv(convergence) {}
...
inline float block_convergence(const Context& ctx, size_t bx, size_t by) {
    ...
    for (size_t y = by * TILESIZE; y < glm::min(ctx.fbo.h, (by + 1) * TILESIZE); ++y) {
        for (size_t x = bx * TILESIZE; x < glm::min(ctx.fbo.w, (bx + 1) * TILESIZE); ++x) {
            const float err =
                luma(glm::abs(ctx.fbo.color(x, y) - ctx.fbo.even(x, y))) / fmaxf(1e-5f, luma(ctx.fbo.color(x, y)));
            count += 1;
...
#pragma omp parallel for
        for (int by = 0; by < TILES_H; ++by) {
            for (int bx = 0; bx < TILES_W; ++bx) {
                ...
                if (conv > ctx.ERROR_EPS) unconverged.push(by * TILES_W + bx, conv);
            }
        }
...
#pragma omp parallel
    ...
    ctx.fbo.tonemap();
#ifdef WITH_OIDN
    if (ctx.BEAUTY_RENDER) ctx.fbo.denoise();
#endif
