...
struct LockTimer : public Timer {
    ...
    void stop(const std::string& name) {
        const auto duration = std::chrono::system_clock::now() - starts[name];
        times[name] += std::chrono::duration_cast<std::chrono::nanoseconds>(duration).count();
        mutex.unlock();
    }
