...
SurfaceHit::SurfaceHit(const SkyLight* sky)
    : Hit(false), light(sky) {}

SurfaceHit::SurfaceHit(const Ray& ray, const Mesh* mesh)
    : Hit(true), mesh(mesh), mat(mesh->mat.get()), light(0) {
    assert(mesh);
    assert(mat);
    STAT("hit point lerp");
    ...
    if (!mesh->tcs.empty()) TC = w * mesh->tcs[tri[0]] + u * mesh->tcs[tri[1]] + v * mesh->tcs[tri[2]];
    ...
    // light source hit?
    if (mesh->is_light()) light = mesh->area_light.get();
}

SurfaceHit::SurfaceHit(const glm::vec2& sample, uint32_t primID, const Mesh* mesh)
    : Hit(true), mesh(mesh), mat(mesh->mat.get()), light(0) {
    assert(mesh);
    assert(mat);
...
SurfaceHit::SurfaceHit(const glm::vec3& position, const glm::vec3& normal)
    : Hit(true), P(position), Ng(normal), N(normal), TC(0), area(0), mesh(0), mat(0), light(0) {}
...
VolumeHit::VolumeHit(const glm::vec3& position, const Volume* volume)
    : Hit(true), P(position), volume(volume) {}
