...
Texture::Texture(size_t w, size_t h, const glm::vec3* data)
    : Texture() {
    load(w, h, data);
}

Texture::Texture(const glm::vec3& col)
    : Texture() {
    load(col);
}
...
void Texture::load(const std::filesystem::path& path, bool sRGB) {
    ...
    if (HDR) {
        ...
        if (!img_data) throw std::runtime_error("Failed to load texture: " + path.string());
        texels.resize(w * h);
