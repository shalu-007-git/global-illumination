...
Material::Material(const aiMaterial* material_ai, const std::filesystem::path& base_path) {
    ...
    if (luma(albedo_col) < 1e-4) albedo_col = glm::vec3(spec.r, spec.g, spec.b);
...
void Material::set_light() {
    ...
    if (emissive_strength <= 0.f) emissive_strength = 10.f;
    type = "light";
}
...
void Material::from_json(const json11::Json& cfg) {
    if (cfg.is_object()) {
        if (cfg["name"].is_string()) name = cfg["name"].string_value();
