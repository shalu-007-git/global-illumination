...
void Timer::stop(const std::string& name) {
    times[name] +=
        std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now() - starts[name]).count();
    counts[name]++;
}
...
std::string Timer::format_time(double ms_elapsed, double ms_total, uint64_t count) {
    ...
    time << std::fixed << std::right << std::setw(20)
         << std::to_string(elapsed_minutes) + "m, " + std::to_string(elapsed_seconds) + "s, " +
                std::to_string(uint64_t(ms_elapsed) % 1000) + "ms";
    time << std::fixed << std::right << std::setw(5) << " [" << ms_elapsed / ms_total * 100 << "%]";
    time << std::fixed << std::right << std::setw(10) << "(" << ms_elapsed / count * 1000000 << "ns * "
         << count / 1000000.0 << "M calls)";
    return time.str();
}
...
StatTimer::StatTimer(const std::string& name)
    : name(name) {
    timer.start(name);
}
