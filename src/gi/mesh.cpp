...
void alphamapFilter(const RTCFilterFunctionNArguments* args) {
    ...
    for (uint32_t i = 0; i < args->N; ++i) {
        ...
        // perform alpha test
        if (mesh->mat->alphamap(TC) < .1f) args->valid[i] = 0; // reject hit
    }
}
...
Mesh::Mesh(RTCDevice& device, RTCScene& scene, const std::shared_ptr<Material>& mat, const aiMesh* ai_mesh)
    ...
    if (has_tcs) tcs.reserve(numVertices + 1);
    ...
    rtcSetSharedGeometryBuffer(
        geom, RTC_BUFFER_TYPE_VERTEX, 0, RTC_FORMAT_FLOAT3, vbo.data(), 0, sizeof(glm::vec3), vbo.size()
    );
    rtcSetSharedGeometryBuffer(
        geom, RTC_BUFFER_TYPE_INDEX, 0, RTC_FORMAT_UINT3, ibo.data(), 0, sizeof(glm::uvec3), ibo.size()
    );
    ...
    if (has_tcs)
        rtcSetSharedGeometryBuffer(
            geom, RTC_BUFFER_TYPE_VERTEX_ATTRIBUTE, 1, RTC_FORMAT_FLOAT2, tcs.data(), 0, sizeof(glm::vec2), tcs.size()
        );
...
Mesh::Mesh(RTCDevice& device, RTCScene& scene, const std::shared_ptr<Material>& mat, const par_shapes_mesh* par_mesh)
    ...
    for (uint32_t i = 0; i < numVertices; ++i) {
        ...
        // tex coords
        if (has_tcs) tcs.emplace_back(par_mesh->tcoords[2 * i + 0], par_mesh->tcoords[2 * i + 1]);
        // compute AABB
        bb_min = min(bb_min, vbo[i]);
        bb_max = max(bb_max, vbo[i]);
    }
    ...
    for (uint32_t i = 0; i < numTriangles; ++i)
        ibo.emplace_back(
            par_mesh->triangles[3 * i + 0], par_mesh->triangles[3 * i + 1], par_mesh->triangles[3 * i + 2]
        );
