...
class Camera {
  public:
     ...
     */
    Ray view_ray(
        uint32_t x, uint32_t y, uint32_t w, uint32_t h, const glm::vec2& pixel_sample = glm::vec2(.5f),
        const glm::vec2& lens_sample = glm::vec2(.5)
    ) const;
