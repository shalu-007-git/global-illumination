...
class RNG {
  public:
    ...
    inline static float uniform_float() {
        ...
        return std::min(
            instance().uniform_float_distribution(instance().per_thread_rng[omp_get_thread_num()]), 0.99999f
        );
    }
     ...
     */
    template <typename T>
    inline static void shuffle(std::vector<T>& target) {
  ...
  private:
    RNG()
        : per_thread_rng(omp_get_max_threads()) {
...
template <>
inline glm::vec2 RNG::uniform<glm::vec2>() {
    return glm::vec2(uniform_float(), uniform_float());
}
template <>
inline glm::vec3 RNG::uniform<glm::vec3>() {
    return glm::vec3(uniform_float(), uniform_float(), uniform_float());
}
template <>
inline glm::vec4 RNG::uniform<glm::vec4>() {
...
template <>
inline glm::uvec2 RNG::uniform<glm::uvec2>() {
    return glm::uvec2(uniform_uint(), uniform_uint());
}
template <>
inline glm::uvec3 RNG::uniform<glm::uvec3>() {
    return glm::uvec3(uniform_uint(), uniform_uint(), uniform_uint());
}
template <>
inline glm::uvec4 RNG::uniform<glm::uvec4>() {
