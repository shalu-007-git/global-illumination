...
class Buffer {
  public:
    ...
    Buffer(size_t w, size_t h = 1, size_t d = 1)
        : w(w), h(h), d(d), mem(w * h * d) {}
    // copy constructor
    ...
    T& operator()(size_t x, size_t y = 0, size_t z = 0) { return mem[z * w * h + y * w + x]; }
    const T& operator()(size_t x, size_t y = 0, size_t z = 0) const { return mem[z * w * h + y * w + x]; }
